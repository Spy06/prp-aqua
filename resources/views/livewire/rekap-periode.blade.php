<div class="space-y-6" id="rekap-periode-container">
    {{-- Header --}}
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-semibold text-zinc-900 dark:text-zinc-100">Rekap Periode</h2>
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('export.excel', $queryParams) }}"
               target="_blank"
               class="inline-flex items-center gap-2 px-3 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition">
                <flux:icon.table-cells variant="outline" class="w-4 h-4" />
                Export Excel
            </a>
            <a href="{{ route('export.pdf.rekap', $queryParams) }}"
               target="_blank"
               class="inline-flex items-center gap-2 px-3 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
        <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-5">
            ...
            <p class="text-3xl font-bold text-zinc-900 dark:text-zinc-100">{{ $total }}</p>
        </div>
        <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-5">
            ...
            <p class="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{{ $perStatus['open'] }}</p>
        </div>
        <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-5">
            <div class="flex items-center gap-2 mb-1">
                <div class="w-2.5 h-2.5 rounded-full bg-blue-500"></div>
                <p class="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">In Progress</p>
            </div>
            <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $perStatus['in_progress'] }}</p>
        </div>
        <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-5">
            <div class="flex items-center gap-2 mb-1">
            ...
            <p class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ $perStatus['closed_pending_qa'] }}</p>
        </div>
        <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-5">
            <div class="flex items-center gap-2 mb-1">
            ...
            <p class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $perStatus['closed_acc'] }}</p>
        </div>
    </div>
